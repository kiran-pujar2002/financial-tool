  ...
  async function handlePayment() {
    ...
    try {
      ...
      const razorpayCheckout = new window.Razorpay({
        ...
        modal: {
          ondismiss: () => {
            // Customer closed the modal without paying — not an error, just stop the spinner.
            setActionPending(false);
          },
        },
      });
            ...
            <div className="border hairline rounded bg-paperRaised p-6 flex items-center justify-between">
              ...
              {isCompleted ? (
                <button
                  ...
                  className="bg-ledger text-white rounded px-4 py-2 text-sm font-medium hover:bg-ledger-dark transition-colors disabled:opacity-50"
                >
                  {actionPending ? 'Downloading…' : 'Download PDF'}
                </button>
              ) : isPaid ? (
                <button
